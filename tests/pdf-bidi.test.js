...
function shapeForJsPdf(text) {
  // Phase 23 (23-12): bracket mirroring restored per UAX-BD16 — matches Word/dir=rtl
  // browsers (Sapir's UAT preference for native HE readers).
  // G16 (Plan 23-12): getMirroredCharactersMap requires levels.levels (raw
  // Uint8Array), not the wrapper. See canonical impl in assets/pdf-export.js.
  if (!text) return '';
...
var VECTORS = [
  ...
  { id:  4, label: 'Hebrew + brackets mirrored (23-12, UAX-BD16)', input: 'הפגישה (חשובה) הסתיימה',             expected: 'המייתסה (הבושח) השיגפה' },
  ...
  { id: 12, label: 'Hebrew + square brackets (mirrored)',      input: 'רישום [important] כאן',                  expected: 'ןאכ [important] םושיר' },
];
