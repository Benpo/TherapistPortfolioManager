...
- **D-10 (effort guardrail):** Because "broad" on heavily UI-tangled files is the highest-effort option, the **research step must produce a concrete behavior inventory** (the list of observable behaviors to pin, per module) **with an effort estimate**, so Ben can right-size at plan time if it's bigger than expected. Broad ≠ unbounded — it's "cover the observable behaviors," not "chase line coverage." Format calibrated with Ben on a sample (snippet-settings IIFE) 2026-06-26: per-function row with observable behavior, Tech×Biz scoring, and a Cheap/jsdom/Skip tier; cross-check existing `tests/` so already-covered helpers aren't re-tested.

### Analysis deliverables — forward-looking (MANDATORY, agreed 2026-06-26)
The research analysis is not just "what to test" — for **every caught section** it must also record **whether it should be improved going forward, and where that improvement belongs.** Three required outputs on top of the behavior inventory:
- **D-13a — Full-file coverage map.** A table of ALL `assets/*.js` (lines × existing test-file count) with an explicit **in-scope / out-of-scope** verdict for Phase 30, so the scoping is a *conscious choice*, not an omission. (Ben caught that the first pass showed a slice without the map.) The two god modules are in scope; everything else is out — but listed, with the reason.
- **D-13b — Per-finding forward-improvement classification.** Each function/section flagged during characterization gets one of: `keep-as-is` / `consolidate-in-P31` (glue duplication etc.) / `extract-later (backlog)` / `backfill-tests-later (backlog)`. So the inventory is a decision list, not just observations. Example already found: `getCurrentLang` (5-line wrapper around `window.App.getLanguage()`) → Skip-for-test + `consolidate-in-P31`; `t()` duplicated across 5 files, `showToast` across 2 → `consolidate-in-P31` (god modules) / backlog (app-wide).
- **D-13c — Glue-duplication / consolidation-candidate list.** The cross-IIFE duplicated thin glue, surfaced explicitly for Phase 31 (god modules) and the post-v1.2 backlog (app-wide).

- **D-14 (scope boundary — what is NOT Phase 30/31):** The analysis WILL surface coverage gaps and extraction candidates in OTHER files (notably `app.js` = 1,474 lines / 6 tests, `license.js` = 568 / 0 tests, plus `backup.js`/`pdf-export.js`/`db.js`). These are **real and dangerous but OUT of v1.2 scope** — Phase 30 char-tests only the two god modules; Phase 31 refactors only those two. The broader extraction + test-coverage health work is captured as a **backlog / roadmap-outlook item** (`.planning/todos/pending/2026-06-26-broader-extraction-and-test-coverage-health.md` + ROADMAP `## Backlog`), likely a future "v1.3 Codebase Health II". The research must NOT silently expand into these — it lists them in the coverage map (D-13a) and routes them to backlog, nothing more.
