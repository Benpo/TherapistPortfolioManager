...
**Depends on**: Nothing (fully independent — the AI native-translation panel produces the strings; no human-translator dependency, per D-02)
...
- [x] 35-03-PLAN.md — Demo home chrome convergence + terminology sweep (D-01/D-07)
- [x] 35-04-PLAN.md — Seed refresh: Heart-Shield arc + relative dates, human-approved (D-03/04/05/06/07)
- [x] 35-05-PLAN.md — Delete demo-hints.js (three coordinated edits) + SW cache reconcile (D-08)
...
- [x] 36-01-PLAN.md — batch-1: db.js (new IDB-choke-point banner) + overview.js (refine + de-phase bug-ticket; PUBLIC SURFACE names both globals) + sessions.js (new banner) [3 files]
...
- [x] 36-03-PLAN.md — mid: crashlog.js + report.js (OBS-01/OBS-02 stripped to prose per D-07) + disclaimer.js + snippets-seed.js + crop.js [5 files]
- [x] 36-04-PLAN.md — small/chrome/sw/stubs: version.js (light de-phase, VER-ids stripped) + add-client.js + shared-chrome.js + reporting.js + globe-lang.js + md-render.js + demo-seed.js + demo.js + sw.js (comment-only, no cache bump) [9 files; i18n.js dropped — already compliant]
...
- [x] 37-02-PLAN.md — Wave 0: personalization-surface RED tests (picker/F4/backup/birthdate + tab deep-link)
- [x] 37-03-PLAN.md — Wave 1: date engine core (window.DateFormat) + App.formatDate delegate + 8-page/sw/test-env wiring
...
- [x] 37-10-PLAN.md — Wave 5: RED behavior tests (Session Format multi-select predicate + Heart-Wall predicate + Overview header-sort↔dropdown sync) — pins the DOM/selector contract, fails RED until Waves 7–8
...
- [x] 38-02-PLAN.md — Wave 1: Extend suites — overview sort (blanks-to-bottom/most-recent), export date + date-only, demo seed, snapshot-revert capture (NEXT-03, NEXT-04, NEXT-06, NEXT-07, NEXT-08)
- [x] 38-03-PLAN.md — Wave 1: i18n keys ×4 languages — table/sort/form/overdue (NEXT-01, NEXT-03, NEXT-04, NEXT-05)
...
- [x] 38-12-PLAN.md — Wave 2: Error-tone toast + auto-focus — backward-compatible `showToast(message, key, options)` (warning tone, longer duration, scroll/focus field) + `.toast--error` (dark-safe warning tokens); migrate session/client form error toasts; behavior test + on-device verify (UAT test 8) (NEXT-01, NEXT-08)
