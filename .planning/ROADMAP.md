...
- [x] 40-02-PLAN.md — `attention-coordinator.js` core (data-driven precedence registry, one-per-session, demo-off, `beforeinstallprompt` capture) + Variant-B welcome overlay + welcome CSS [wave 2]
...
- [x] 40-07-PLAN.md — Remove redundant Help nav pill (+ removal-regression test) + `.help-entry-item` native-button reset (no more "preselected" row) [wave 1]
- [x] 40-08-PLAN.md — Approved two-paragraph EN welcome copy (`subtitle` P1 + new `subtitle2` P2) + "Onboarding screen" menu label + he/de/cs empty parity stubs (translation deferred to 42.1) [wave 2]
...
- [x] 41-01-PLAN.md — Tour copy: ~30 help.tour.* keys in all 4 locales + RED parity gate [wave 1]
- [x] 41-02-PLAN.md — 10 data-tour anchors on real chrome + anchor-presence rot-guard test [wave 1]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 41-03-PLAN.md — Tour engine core: tour.js/tour.css render + degradation + cross-page resume (+ 3 behavior tests) [wave 2]

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 41-04-PLAN.md — Engine polish: language re-render + exit choice + finish card + bottom-sheet [wave 3]

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 41-05-PLAN.md — Launch wiring: "?" tour row (demo-gated) + welcome CTA rewire + coordinator reminder + resume hook [wave 4]
- [x] 41-06-PLAN.md — Offline: tour.js/tour.css script/link on 4 pages + sw.js precache [wave 4]

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 41-07-PLAN.md — Playwright-WebKit RTL/geometry gate + real-Safari UAT checkpoint [wave 5]

**Wave 6** *(gap-closure — UAT remediation per 41-STORYLINE.md, parallel; build on the shipped tour)*

- [x] 41-08-PLAN.md — RTL blocker + first-paint geometry fix + strengthened WebKit probe (UAT gaps 2/3/4) [wave 6]
...
- [x] 41-11-PLAN.md — Storyline copy in 4 locales + parity gate realign (gaps 1/5/6/7) [wave 6]
...
- [x] 41-12-PLAN.md — "?" menu trim to 3 items + relabel + ONBD-02/D-17 reversal note (gap 8) [wave 7]
...
- [x] 42-01-PLAN.md — Wave 0: RED popup + coordinator-tour-guard behavior tests (gating/dismiss/tour-guard)
...
- [x] 42-06-PLAN.md — Wave 1: coordinator tour-active suppression guard (run() shows nothing during a tour)
- [x] 42-07-PLAN.md — Wave 1: standalone changelog page (changelog.html + changelog.js + changelog.css)
- [x] 42-08-PLAN.md — Wave 1: 10 changelog.*/whatsNew.* i18n chrome keys in all 4 locales

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 42-09-PLAN.md — Wave 2: every-page popup wiring (7 pages) + sw.js two-array offline precache
- [x] 42-10-PLAN.md — Wave 2: entry points ("?" row, help rail link, footer version link) + demo hiding
...
**Scope notes:** Translation happens AFTER the EN corpus stabilizes (post-P42) — one pass, no double-translation. Pipeline per the D-12/D-19 precedent: agent translation grounded in the shipped EN + each locale's existing i18n register conventions (HE infinitive/plural-imperative house style, DE Sie, CS formal) → native-speaker agent gates (Sonnet) → Sapir human read for Hebrew (CS ships on the Phase-37 accepted-risk precedent; external review only on a deliberate CS push). Per-locale integrity tests mirroring `tests/help-integrity.test.js` ({ui:key} resolution against each locale file, forbidden-words, structure parity with EN). Scope decision: Ben 2026-07-08 — L10N-01 moved from v2 deferral into v1.3 (EN-only help was never his intent for a Hebrew-first user base). Must land BEFORE Phase 43 so the docs hard-gate covers translated content from day one.
...
- [x] 42.1-01-PLAN.md — RED-first per-locale integrity gates (help + changelog)
- [x] 42.1-02-PLAN.md — RED-first extensions: tour \n\n, welcome subtitle2, precache (6 files), What's-New popup localization
...
- [x] 42.1-06-PLAN.md — Changelog full history HE/DE/CS (changelog-content-*.js)
- [x] 42.1-07-PLAN.md — Welcome + tour chrome: re-translate subtitle/subtitle2 + re-derive 11 stale tour keys (i18n-{he,de,cs}.js)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 42.1-08-PLAN.md — Loader + offline wiring (help.js localeSections, whats-new.js entries, script tags, sw.js precache)
- [x] 42.1-09-PLAN.md — Native-speaker register gate HE/DE/CS (whole-corpus per locale)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 42.1-10-PLAN.md — HE real-browser render verification (Chrome + WebKit) + Sapir Hebrew read (blocking checkpoint)
...
- [x] 43-02-PLAN.md — Foundation: shared vm content loader (scripts/lib/) + 5 test renames (D-22) with before/after grep post-condition + TESTING map/count [wave 1]

**Wave 2** *(blocked on Wave 1)*

- [x] 43-03-PLAN.md — covers[] backfill (D-19) + gen-help-map.js + generated HELP-MAP.md, coherent in one commit (D-13/D-17 #1) [wave 2]
- [x] 43-04-PLAN.md — D-20 covers[] EN-only: strip he/de/cs + delete the locale covers-parity block [wave 2]

**Wave 3** *(blocked on Wave 2)*

- [x] 43-05-PLAN.md — role-table.js (written GATE-03 definition, OD-1 watch-code-only) + invariants.js (4 D-17 checks) + wire renamed integrity tests [wave 3]
...
| 43. Docs-Maintenance Hard Gate | v1.3 | 10/10 | In Progress | - |
</content>
</invoke>
