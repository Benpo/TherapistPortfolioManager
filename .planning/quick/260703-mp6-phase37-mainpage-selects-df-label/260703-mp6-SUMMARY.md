...
- **RTL alignment bug found + fixed during UAT review (`a227661`, follow-up):** Ben noticed in Hebrew the closed select's selected VALUE sat ~2.6rem in from the right edge, so it didn't line up under its right-aligned label. Root cause was in the *shipped* `.select-modern` (commit `ebe502f`, not E1): the base rule uses LOGICAL `padding-inline: 1rem 2.6rem` (auto-flips in RTL), but the `html[dir="rtl"]` override manually swapped it to `2.6rem 1rem`, double-flipping and pushing the chevron-clearance to the wrong (right) side. Fix = drop the redundant padding line, keep only the chevron `background-position: left 1rem center`. Value now hugs the right, aligned under the label; chevron clearance verified on long values; LTR unaffected. Corrects ALL modern selects in Hebrew (settings/add-session too), not just the E1 filters. Verified via label-vs-value edge-guide renders against the real app.css.
