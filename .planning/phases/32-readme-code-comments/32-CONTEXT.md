...
- **D-02 — Cloud Claude Code is now a primary workflow.** It wasn't available when the app started. Consequences: the README lives **in the repo** so a cloud agent reads it as context (reinforces D-03); drop the multi-machine-collaboration emphasis; **keep** local-serve instructions (localhost is required for the Web Crypto secure-context backup features).
...
- **D-04 — Stop shipping the README publicly.** Remove the `cp README.md deploy-staging/` line from `.github/workflows/deploy.yml` so the maintainer README stays repo-only. **This is the only production-adjacent change in the phase.** (Repo is private; the Cloudflare copy was unlinked — no loss. Avoids publishing maintenance/deploy detail at the product URL.)
...
- **The README doubles as agent-context.** With cloud Claude Code as a primary workflow, an in-repo README whose "rules an agent must not break" section is precise is the single highest-leverage maintenance artifact — write it for a capable agent + Ben, not for a non-technical first-timer.
