...
2. **User-configurable prefix** — default `;`, editable in Settings.
   - Current: No prefix concept exists.
   - Target: A setting `snippetPrefix` lives in the existing `therapistSettings` IDB store, defaulting to `;`. Settings UI exposes a single-character text input for it. Validation: exactly 1 character; must not be whitespace; must not be a letter or digit (would clash with keyword characters); must not be a quote or angle bracket (would clash with HTML/markdown). Changing the prefix globally re-keys trigger detection — no per-snippet prefix override.
   - Acceptance: User changes prefix from `;` to `\` in Settings. After save, typing `\<trigger>` + word-boundary fires expansion; typing `;<trigger>` + word-boundary no longer fires.
...
- **Prefix character constraints.** Prefix must be exactly 1 character; cannot be whitespace, letter, digit, quote (`"`, `'`, `` ` ``), or angle bracket (`<`, `>`). Recommended set: `;`, `:`, `\`, `/`, `~`, `#`, `@`, `$`, `%`, `^`, `&`, `*`, `|`. Default: `;`.
...
- [ ] Changing the snippet prefix in Settings from `;` to `\` makes `\<trigger>` fire and `;<trigger>` no longer fire, with no app reload required.
