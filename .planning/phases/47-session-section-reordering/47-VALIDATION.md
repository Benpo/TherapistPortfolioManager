...
| 47-01 T3 | 47-01 | 1 | ORDR-07 | T-47-03 | severity widget tap-again-to-clear (D-20); getSeverityValue returns Number / null (unchanged) | unit | `node tests/47-severity-clear.test.js` | ❌ create in 47-01 | ⬜ pending |
...
| 47-05 T2 | 47-05 | 2 | ORDR-06/07 (D-08/D-14/D-15/D-21) | T-47-06 | unrated excluded from export via the buildRenderInputs PDF-input filter (fully-unrated → no row, all-unrated → block omitted, partial → row kept); no literal placeholder leaks; topics-without-severity emits the topic-name section at the issues slot (G-2); switch-off hides the sub-option + excludes severity from the filtered builder + PDF payload (G-3); topics re-check restores the sub-option default (G-12) | unit | `node tests/47-severity-unrated.test.js` | ❌ create in 47-05 | ⬜ pending |
| 47-09 T1 | 47-09 | 3 | ORDR-04 (Pattern 3) | T-47-08b | `deriveSeverityAfterSections` EDIT-AWARE ordinal (saved order ∩ parsed editor headings; a Step-2 heading deletion shifts the block, G-8) correct for varied saved orders; pdf-export.js render loop untouched | unit (pure) | `node tests/47-severity-position.test.js` | ❌ create in 47-09 | ⬜ pending |
| 47-09 T2 | 47-09 | 3 | ORDR-04/07 (D-13/D-21) | T-47-06b | clipboard `buildSessionMarkdown` emits saved order + same topics/sub-option/switch gating; unrated topic copies as name only — no unrated rating line, no NaN change line (G-1) | unit | `node tests/47-severity-unrated.test.js` | ❌ extended in 47-09 T2 | ⬜ pending |
...
| 47-07 T1/T2 | 47-07 | 3 | ORDR-06/07 | T-47-03/12 | cleared (null) start rating round-trips + never blocks save (validateIssues unchanged, G-17) + auto-hides the end-of-session rating keyed on emptiness (D-22); severity-off hides start-rating column on data-free sessions, keeps both visible+badged on past sessions with data (G-11) | unit (jsdom) | `node tests/47-severity-form.test.js` | ❌ create in 47-07 | ⬜ pending |
| 47-10 T1/T2 | 47-10 | 2 | ORDR-07 (D-23) | T-47-14 | fully-unrated topic renders name only (no "(- -> -)" suffix) in Sessions History + client-overview; partial/full unchanged; overview averages untouched | unit (jsdom) | `node tests/47-view-unrated.test.js` | ❌ create in 47-10 | ⬜ pending |
...
- [ ] `tests/47-severity-clear.test.js` — created in **47-01 T3** (widget tap-again-to-clear; getSeverityValue Number|null, D-20)
...
- [ ] `tests/47-severity-unrated.test.js` — created in **47-05 T2** (unrated omission via the buildRenderInputs PDF-input filter: fully-unrated → no row, all-unrated → block omitted, partial → row kept, D-21; + topics-without-severity emission (G-2) + switch-off exclusion (G-3) + re-check default (G-12)), EXTENDED in **47-09 T2** (clipboard buildSessionMarkdown: saved-order + gating + unrated-copies-as-name-only, no NaN — G-1)
- [ ] `tests/47-severity-position.test.js` — created in **47-09 T1** (`deriveSeverityAfterSections` edit-aware ordinal — saved order ∩ parsed editor headings, G-8)
- [ ] `tests/47-order-backup-roundtrip.test.js` — created in **47-06 T1** (encrypted round-trip + restore sanitize)
- [ ] `tests/47-severity-form.test.js` — created in **47-07 T1/T2** (cleared/unrated save→round-trip + end-of-session-rating auto-hide keyed on emptiness (D-22) + severity-off column incl. past-session-with-data badge mirror (G-11))
- [ ] `tests/47-view-unrated.test.js` — created in **47-10 T1/T2** (fully-unrated topic → name only in Sessions History + client-overview; partial/full unchanged; overview averages untouched, D-23)
