...
> **SUPERSESSION NOTE (Ben, 2026-07-23, plan review — CONTEXT D-19…D-23):** the "— (skip)" 11th severity value is REMOVED from scope. Ratings were never validation-mandatory in the shipped code, so unrated (`null`) already carries the skip meaning. Everything in this spec about the — pill is superseded: the 11th-value styling (§Severity scale), the skipped-rating hint copy, the Error-state row's "— satisfies validation" sentence (validation never inspected ratings — no dead-end exists), and Interactions 8/8a's pill mechanics. Replacements: tap-again-to-clear on the active pill (D-20); unrated topics omitted from every output incl. the PDF bar block and the History/overview views (D-21/D-23); end-of-session block lists only start-rated topics (D-22). Interaction 8a's export-omission INTENT stands, keyed on unrated instead of —. The mockup's — pill panel is likewise historical.
