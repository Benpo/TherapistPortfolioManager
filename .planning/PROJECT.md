...
**Phase 47 (Session-Section Reordering) complete 2026-07-24** — the session form is now shaped per therapist: every section reorders by drag or accessible arrows in Settings (one shared `App.sanitizeOrder` clamp guards every write path — UI, save, backup restore, incl. structural hardening against crafted orders), groups are data with renamable titles that render on the form, and the order drives the form, both export builders, and the PDF severity-block placement from one saved-order source (closing the 260615 fragmented-order bug class). Severity became unrated-by-default (null, tap-again-to-clear, clear-voids-pair) with an app-wide on/off switch; unrated topics are omitted from exports and render name-only in views. 13/13 plans (10 + 3 preprod-UAT gap plans) + a 15-commit fix layer from six review rounds (dual-opus + backup-completeness + existing-customer-risk + a Fable whole-diff sweep whose blocker — group renames not rendering on the form — was fixed and real-browser-verified). Two hard-won engineering lessons memorized: insertBefore releases pointer capture in all engines (capture-free drag, real-engine probes; jsdom false-greens the class), and preprod's 24h asset cache demands private-window retests. Verifier passed 8/8 by execution; suite 218/218; APP_VERSION 1.5.0 + four-locale help/changelog live on pre-prod at `aa7d848`-era; production push (v1.5.0) pending Ben's explicit call. Next: Phase 48 (mobile-pass-&-validation-polish).

**Phase 46.1 (Preview/Edit Experience Redesign) complete 2026-07-20** — the rich-text editing experience is finalized on both surfaces: an in-place preview swap (the framed PREVIEW view takes the editor's box — one surface at a time) with a pinned current-state Edit/Preview switcher and an inert formatting bar while previewing (Ben's "dimmed means dead" amendment), export Step-2 content-driven sizing with an always-pinned toolbar and a lossless Back/Continue round-trip (discard asked only at the section-selection change), plus four UAT rounds layered in-phase: the help corpus restructured around a searchable "Formatting session notes" topic, a state-based bold/italic toggle implementing the ratified 11-clause behavior contract (un-bold-not-nest, per-line multi-line wrap, marker-collision swap), pure deletions moved to execCommand('delete') (list-exit was inescapable in real WebKit), leave-page guards on all seven same-tab nav triggers, and a real note-heading size hierarchy (22/18/16). Verifier passed 10/10 by execution; code review 1 crit + 3 warn all fixed same cycle (3 infos open in `46.1-REVIEW.md`); parent Phase-46 ledger fully closed (16/16 gaps); suite 205/205. Closes Phase 46's rejected preview concept (gaps 14/15). Next: Phase 47 (session-section-reordering); v1.4.0 production push pending Ben's explicit call.
...
---
*Last updated: 2026-07-24 after Phase 47 — session-section reordering complete, phase verified 8/8, v1.5.0 staged on pre-prod. App live & sold.*
