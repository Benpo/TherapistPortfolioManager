...
**Progress:** Phase 39 (Help Center & "?" Entry Point) complete 2026-07-07 — HELP-01..07 validated: offline `help.html` (12-section workflow-spine IA, live-label `{ui:key}` chips), persistent "?" popover on every page, empty-state coaching deep-links, SW-precached with the APP_VERSION 1.3.0 cache roll, D-19 content gates run with all findings applied (Sapir's final rendered-page read pinned as a tracked todo). Phase 40 (First-Run Welcome & Onboarding Coordinator) complete 2026-07-08 — ONBD-01..04 validated: single attention-surface coordinator (one governed surface per session, written precedence order), branded first-run welcome overlay (tour / explore-myself, Esc-dismiss, focus-trapped, re-openable from "?"), non-nagging per-browser install nudge (Chromium `beforeinstallprompt` with late-event re-arm + macOS-Safari Add-to-Dock pointer), legacy iOS banner retired. Went through UAT (5 gaps diagnosed → gap-closure round 40-06/07/08 → 4/4 must-haves re-verified, 2 on-device human checks passed). EN copy canonical; HE/DE/CS translation of the new strings deferred to Phase 42.1. Phase 42.1 (Help & Onboarding Translation HE/DE/CS) complete 2026-07-10 — L10N-01 validated: full HE/DE/CS mirrors of the help body (`help-content-{he,de,cs}.js` + `localeSections()` merge with per-section EN fallback), full-history changelog siblings + locale-aware What's-New popup on all 9 chrome pages, welcome/tour/chrome i18n resynced to the final EN (11 stale tour keys re-derived; 17 new chrome/screen keys ×4 locales), all 6 locale files SW-precached, RTL unlocked on the help page (computed-direction + chevron-mirror browser-verified in Chromium + WebKit); native-speaker register gates ran per locale and Sapir/Ben's blocking Hebrew read passed with corrections applied (broader HE polish pass deferred as a tracked todo). Verification 14/14; code review 0 crit / 2 warn, both fixed. Still open in-milestone: Phase 41 held at UAT gap-closure; Phase 42 awaiting `/gsd-verify-work 42`. Next: Phase 43 (Docs-Maintenance Hard Gate).
...
**Key context:** Phase 26's `26-UI-SPEC.md` + D-01..D-15 are the design seed, verified and adapted against the current app (terminology renames, deleted tour-engine template, post-P26 features, first-run collisions — full reconciliation was done 2026-07-06). Open design questions (help-page IA variant, tour anchors, popup mechanics, gate mechanism, first-run precedence, etc.) are decided in discuss-phase, not pre-answered. Hard constraints hold: zero-network/offline, zero production deps (bespoke tour engine), 4 languages + RTL (EN content canonical; HE/DE/CS translation of all v1.3 copy = Phase 42.1, moved into scope from the original v2 deferral by Ben 2026-07-08). Two pending todos are absorbed and close at ship: `2026-03-24-pwa-install-guidance-and-user-manual`, `2026-05-15-in-app-onboarding-overview-help`.
...
---
*Last updated: 2026-07-10 — Phase 42.1 complete: full HE/DE/CS localization of the v1.3 help/onboarding/changelog surfaces (L10N-01), RTL browser-verified, Sapir/Ben Hebrew read passed with corrections applied. Prior: Phase 40 complete 2026-07-08 (welcome + coordinator + install nudge, ONBD-01..04); Phase 39 complete 2026-07-07 (help center + "?" entry, HELP-01..07). Milestone v1.3 "In-App Help, Onboarding & Changelog" started 2026-07-07; three pillars (help system, in-app changelog, docs-maintenance hard gate); rich-text editor deferred to v1.4.*
