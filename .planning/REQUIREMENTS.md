...
- [x] **CHLG-02**: User can browse a persistent changelog page inside the help center — reverse-chronological, grouped by version + release date, plain-language benefit-led entries (New / Improved / Fixed register, no dev jargon)
- [x] **CHLG-03**: One structured, i18n-capable in-app data source drives both the popup (latest entry) and the page (history) — never forked, never scraped from git, no second version constant
- [x] **CHLG-04**: v1.3's own release notes ship as the first changelog entry (self-hosting proof of the pipeline; backfill depth decided in discuss-phase)
