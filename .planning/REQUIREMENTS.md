...
- [x] **ORDR-08**: Group renames (D-05) — group header rows in the Settings reorder list carry the same ✎ rename (+ revert) pattern that sections have; group title overrides persist in the order sentinel (group-as-DATA, so the future group-management phase plugs in with zero migration). Group creation/dissolution/move-between stays deferred.
